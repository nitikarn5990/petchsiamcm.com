...
if ($_POST ['submit_bt'] == 'บันทึกข้อมูล' || $_POST ['submit_bt'] == 'บันทึกข้อมูล และแก้ไขต่อ') {
    ...
    if ($orders->updateSQL($arrData, $arrKey)) {
        ...
        foreach ($_POST["p_product_id"] as $key => $p_ids) {

            //แก้ไขการรับสินค้าหลายครั้ง
            $qty = $orders_detail->getDataDesc('qty2', 'product_id = ' . $p_ids . ' AND orders_id = ' . $arrKey['id']);
            if ($arrData['status'] == 'ชำระเงินแล้ว') {

                if ($qty != '') {

                    $arrDataDetail['qty2'] = $qty . ',' . "-";
                } else {
                    $arrDataDetail['qty2'] = "-";
                }
            } else {
            ...
            //  print_r($arrDataDetail['qty2']);
            $arrWhere = [
                "orders_id" => $arrKey['id'],
                "product_id" => $p_ids
            ];
            $orders_detail->updateSQL($arrDataDetail, $arrWhere);

            //  $orders_detail->updateSQL(["qty2" => $arrDataDetail], ["orders_id" => $arrKey['id'], "product_id" => $p_ids]);

            $qty2_data = $orders_detail->getDataDesc('qty2', 'product_id = ' . $p_ids . ' AND orders_id = ' . $arrKey['id']);
            $qty_data = $orders_detail->getDataDesc('qty', 'product_id = ' . $p_ids . ' AND orders_id = ' . $arrKey['id']);
           
            if ($qty2_data != '') {
                $arr_rec = explode(',', $qty2_data);

                if (count($arr_rec) > 0) {
                    if (count($arr_rec) == 1) {
                        if ($arr_rec[0] == '-') {
                            //update status qty of prouduct
                            $products->updateSQL([ "qty" => $products->getDataDesc('qty', 'id =' . $p_ids) - $qty_data ], [ "id" => $p_ids]);
                        }
                        ...
                        //   die();
                        if($arr_rec[$arr_before] != $arr_rec[$arr_last]){
                            //ถ้ามีการเปลี่ยนแปลงที่ไม่ซ้ำกันให้ เปลี่ยนสถานะ qty
                            if ($arr_rec[$arr_last] == "+") {
                                 $products->updateSQL([ "qty" => $products->getDataDesc('qty', 'id =' . $p_ids) + $qty_data ], [ "id" => $p_ids]);
                            }
                            if ($arr_rec[$arr_last] == "-") {
                                ...
                                <div class="da-form-item large">
                                    <div class="span12">
                                        ...
                                        while ($row = $db->FetchArray($query)) {
                                            ...
                                            ?>
                                            <div class="span5">
                                                <input type="hidden" name="p_qty[]" value="<?php echo $row['qty'] ?>">
                                                <input type="hidden" name="p_product_id[]" value="<?php echo $row['product_id'] ?>">
                                                <img
